...
class Verifier(object):
    ...
    def verify(self, manager, uri, response, respdata, args):  # @UnusedVariable
        ...
        if String.valueOf(response.status) not in statusCodes:
        ...
        for jpath in exists:
            if jpath.find("~$") != -1:
                ...
                path, value = jpath.split("~~")[0], null
            else:
                path, value = jpath, null
            try:
                jp = JSONMatcher(path)
            except Exception:
                result = false
                ...
                try:
                    ...
                    if value and value not in map(lambda x: null if x is null else String.valueOf(x), jobjs):
                        result = false
